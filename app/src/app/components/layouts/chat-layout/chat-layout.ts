...
export class ChatLayout implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      if (chatId) {
        this.chatService.findChattedWithUser(chatId).subscribe({
          next: (response: UserProps) => {
            this.chatService.chattedWithUser.set(response);
          },
          error: (err) => {
            console.log(err);
          },
        });
        this.chatService.findChatById(chatId).subscribe({
          next: (response: ChatProps) => {
            this.chatService.currentChat.set(response);
          },
        });
      }
    });
